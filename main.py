        ...
        try:
            ...
            if src["type"] == "rss":
                ...
                for a in area.find_all("a", href=True):
                    ...
                    if len(tit) > 30:
                        if src["name"] in ["Msociété", "MIncident"]: tit = clean_mainichi_title(tit)
...
def write_html(filename, data, is_archive=False):
    ...
    with open(filename, "w", encoding="utf-8") as f:
        ...
        for a in data:
            f.write(f"<div class='article' data-s='{a['s']}' data-dt='{a['dt']}' data-orig-t='{a['t'].replace(chr(39),chr(32))}'><div style='font-size:0.7rem;color:#1a73e8;font-weight:bold;margin-bottom:5px'>{a['s']} | {a['dt']}</div><a class='art-title' href='{a['l']}' target='_blank'>{a['t']}</a>")
        ...
        # NOUVELLE FONCTION SIMILARITE JAPONAISE
